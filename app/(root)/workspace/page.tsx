...
export default function PropertyGrid() {
    ...
    fetch("https://localhost:5050/workspaces")
      ...
      .then((data: { workspaces: Workspace[] }) => {
        ...
        const maxPrice = Math.max(
          ...formattedWorkspaces.map(w => Math.max(w.shortTermPrice || 0, w.longTermPrice || 0))
        );
        const maxArea = Math.max(...formattedWorkspaces.map(w => w.area || 0));
        const maxCapacity = Math.max(...formattedWorkspaces.map(w => w.capacity || 0));
  ...
  const applyFilters = (
    ...
    filtered = filtered.filter(workspace => {
      const workspacePrice = Math.max(workspace.shortTermPrice || 0, workspace.longTermPrice || 0);
      return workspacePrice >= price[0] && workspacePrice <= price[1];
    });
    
    filtered = filtered.filter(workspace => 
      workspace.area >= area[0] && workspace.area <= area[1]
    );
  ...
  const resetFilters = () => {
    ...
    const maxPrice = Math.max(
      ...workspaces.map(w => Math.max(w.shortTermPrice || 0, w.longTermPrice || 0))
    );
              ...
              <div className="mb-4">
                ...
                >
                  <Slider
                    range
                    min={0}
                    max={Math.max(...workspaces.map(w => Math.max(w.shortTermPrice || 0, w.longTermPrice || 0)))}
                    step={50000}
                    value={priceRange}
              ...
              <div className="mb-4">
                ...
                >
                  <Slider
                    ...
                    value={capacityRange}
                    onChange={(value) => setCapacityRange(value as [number, number])}
                  />
                </ConfigProvider>
                <div className="flex justify-between text-sm">
