...
const Notification = ({ customer }: { customer: Customer }) => {
  ...
  const fetchNotifications = useCallback(async (force = false) => {
    ...
    const now = Date.now();
    if (!force && now - lastFetchTimeRef.current < MIN_API_CALL_INTERVAL) return;
    ...
    try {
      ...
      if (hasNewNotifications || fetchedNotifications.length !== notifications.length) {
  ...
  const markAsRead = async (id: number) => {
    ...
    try {
      ...
      const errorMessage =
        error instanceof Error ? error.message : "Có lỗi xảy ra khi đánh dấu thông báo đã đọc!";
      toast.error(errorMessage, {
        position: "top-right",
        autoClose: 2000,
        hideProgressBar: false,
  ...
  useEffect(() => {
    ...
    // Listen for visibility changes to optimize API calls
    ...
    window.addEventListener(notificationEvents.UPDATED, handleNotificationUpdate);
    window.addEventListener(notificationEvents.MARKED_READ, handleNotificationMarkedRead as EventListener);
    window.addEventListener(notificationEvents.FETCH_REQUESTED, handleFetchRequested);
    window.addEventListener(notificationEvents.BOOKING_SUCCESS, handleBookingSuccess);
    window.addEventListener(notificationEvents.BOOKING_CANCELED, handleBookingCanceled);
    document.addEventListener('visibilitychange', handleVisibilityChange);
    
    return () => {
      window.removeEventListener(notificationEvents.UPDATED, handleNotificationUpdate);
      window.removeEventListener(notificationEvents.MARKED_READ, handleNotificationMarkedRead as EventListener);
      window.removeEventListener(notificationEvents.FETCH_REQUESTED, handleFetchRequested);
      window.removeEventListener(notificationEvents.BOOKING_SUCCESS, handleBookingSuccess);
      window.removeEventListener(notificationEvents.BOOKING_CANCELED, handleBookingCanceled);
