        ...
        <>
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8">
            {displayedDetails.map((detail) => (
              <div
                key={detail.id}
                className="flex flex-wrap items-center gap-4 p-2 rounded-lg border border-gray-200 bg-white hover:shadow-md transition-all duration-300 hover:-translate-y-[2px]"
              >
                <div className="flex items-center justify-center bg-primary/10 text-primary rounded-full w-8 h-8">
                  <Info size={16} />
                </div>
                <p className="flex flex-wrap text-sm font-medium text-gray-800">
                  {detail.detailName}
                </p>
              </div>
            ))}
