...
import { Button, Input, message as antdMessage, Upload, Form, Card, Typography, Space, Spin, Divider, Tag, Timeline } from "antd";
import { PlusOutlined, SendOutlined, InfoCircleOutlined, CommentOutlined, CheckCircleOutlined } from "@ant-design/icons";
...
const ContactChat: React.FC<ContactChatProps> = ({ userId, bookingId, feedbackId }) => {
  ...
  const [ownerResponse, setOwnerResponse] = useState<OwnerResponse | null>(null);
  ...
  const fetchOwnerResponse = async (feedbackId: number) => {
    try {
      ...
      const response = await fetch(`${BASE_URL}/response-feedbacks/feedback/${feedbackId}`);
  ...
  const handleSendMessage = async () => {
    try {
      ...
      if (description.trim() === "" || title.trim() === "") {
        antdMessage.error("Vui lòng nhập tiêu đề và nội dung phản hồi.");
        return;
      }
      ...
      // Update the existing feedback after successful submission
      ...
      <Card className="shadow-lg border-0 overflow-hidden">
        <div className="mb-6">
          <Space direction="vertical" size="small" className="w-full">
            <Title level={4} className="text-gray-800 mb-0">Phản hồi của bạn</Title>
              ...
              children: (
                <div className="bg-gray-50 p-4 rounded-lg mb-4">
                  ...
                  <div className="mb-2">
                    ...
                    <Text className="ml-2">{new Date(existingFeedback.createdAt).toLocaleDateString('vi-VN')}</Text>
                  </div>
                  <div className="mb-4">
                    <Text strong>Không gian làm việc:</Text>
                    <Text className="ml-2">{existingFeedback.workspaceName}</Text>
                  </div>
                  <Divider className="my-3" />
                  <div className="mb-4">
                    <Text strong>Nội dung phản hồi:</Text>
                    <Paragraph className="mt-2 whitespace-pre-line">{existingFeedback.description}</Paragraph>
                  </div>

                  {existingFeedback.imageUrls && existingFeedback.imageUrls.length > 0 && (
                    <div>
                      <Text strong className="block mb-2">Hình ảnh đính kèm:</Text>
                  ...
                  <div className="bg-green-50 p-4 rounded-lg mb-4">
                    <div className="flex justify-between items-center mb-2">
                      ...
                      <Text className="text-gray-500 text-sm">
                        {new Date(ownerResponse.createdAt).toLocaleDateString('vi-VN')}
                      </Text>
                    </div>
                    <Divider className="my-3" />
                    <Paragraph className="whitespace-pre-line">{ownerResponse.description}</Paragraph>
                  </div>
                ),
              }
            ] : loadingResponse ? [
        ...
        {!ownerResponse && !loadingResponse && (
          <div className="bg-amber-50 p-4 rounded-lg flex items-start gap-3 mt-4">
            ...
            <div>
              <Text strong className="text-amber-700">Lưu ý:</Text>
              <Paragraph className="text-amber-700 text-sm mb-0">
                Phản hồi của bạn đang được xem xét. Chúng tôi sẽ liên hệ với bạn sớm nếu cần thêm thông tin.
              </Paragraph>
            </div>
          </div>
        )}
  ...
  return (
    <Card className="shadow-lg border-0 overflow-hidden">
      <div className="mb-6">
        <Space direction="vertical" size="small" className="w-full">
          <Title level={4} className="text-gray-800 mb-0">Gửi phản hồi của bạn</Title>
          <Paragraph className="text-gray-500">
